<script lang="ts">
  ...
  import type { ConflictingField, MergeResolutionWithCustom } from "../../../lib/utils/mergeEngine.js";
  ...
  // Initialize resolutions (only once per open)
  $effect(() => {
    // Guard: nur wenn open wird true UND noch nicht initialisiert
    if (open && !isInitialized && conflicts.length > 0) {
      isInitialized = true;
      resolutions = {};
