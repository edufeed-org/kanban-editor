...
Scope: Automatisches Laden und Live-Aktualisieren von Boards des angemeldeten Users aus Nostr (Kind 30301 / 30302), integriert mit BoardStore, AuthStore, NDK und SyncManager.

Zielsetzung

Diese Anleitung beschreibt präzise, wie der in [`AGENTS.md`](../AGENTS.md) und den Store-Dokumenten definierte Plan konkret im Code umgesetzt wird.
...
Ziel: Live-Updates für Board-Metadaten (Kind 30301) aus Nostr abonnieren und in BoardStore spiegeln.
...
- Prüfen, ob Board bereits bekannt ist:
  - Ist `boardProps.id` in `this.boardIds`?
- Last-Write-Wins:
  ...
  - Wenn `remoteTimestamp` > lokal:
    - Nostr-Version gewinnt → Board im localStorage überschreiben.
...
- Aktualisierung des aktiven Boards:
  ...
  - Wenn das aktualisierte Board das aktuell geladene `this.board.id` ist:
    - `this.board = new Board(boardPropsFromStorageOrNostr);`
